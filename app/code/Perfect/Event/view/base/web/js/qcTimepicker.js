...
function timeToSeconds(t, strict) {
    ...
    t = jQuery.map(t.split(':'), function (part) {
...
function setTime(time, dropdown) {
    if (
        jQuery(dropdown.children).filter(function () {
...
var methods = {
        ...
        return this.filter('input')
            .each(function () {
                var that = this,
                    ...
                    opt,
                    labels = jQuery('label[for="' + that.id + '"]'),
                    options,
                    time;

                options = jQuery.extend({}, jQuery.fn.qcTimepicker.defaults, o);
                ...
                if (options.classes) {
                    if (
                        ...
                        jQuery.each(options.classes, function (i, v) {
                            tSelect.className += ' ' + v;
                        });
                        tSelect.className = jQuery.trim(tSelect.className);
                ...
                if (
                    ...
                    options.step = jQuery.fn.qcTimepicker.defaults.step;
                }
    ...
    hide: function () {
        return this.filter('input')
            .each(function () {
                jQuery('#' + this.getAttribute('data-qctimepicker-id')).hide();
            })
            .end();
    },
    ...
    show: function () {
        return this.filter('input')
            .each(function () {
                jQuery('#' + this.getAttribute('data-qctimepicker-id')).show();
            })
            .end();
    },
 ...
 */
jQuery.fn.qcTimepicker = function (args) {
    if (methods[args]) {
    ...
    jQuery.error('Method ' + args + ' does not exist on jQuery.qcTimepicker');
};
 ...
 * @struct
 */
jQuery.fn.qcTimepicker.defaults = {
    classes: '',
    format: 'H:mm',
    minTime: '0:00:00',
    maxTime: '23:59:59',
